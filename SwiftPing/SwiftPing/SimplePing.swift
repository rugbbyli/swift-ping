...
public class SimplePing {
    ...
    func setTTL(ttl: Int) -> Int32
    {
        var _ttl = ttl
        return setsockopt(CFSocketGetNative(sock), IPPROTO_IP, IP_TTL, &_ttl, socklen_t(MemoryLayout.size(ofValue: _ttl)));
    }
...
      MARK: - Private
	...
	private func validatePing4ResponsePacket(_ packet: inout Data, sequenceNumber: inout UInt16) -> PingReply {
        var reply = PingReply()
        ...
        reply.ipv4Header = IPv4Header(data: packet)
        reply.icmpHeader = icmpHeader
        ...
        reply.valid = icmpHeader.identifier == identifier
	...
	number.
	- returns: true if the packet looks like a reasonable IPv4 ping response. */
	private func validatePing6ResponsePacket(_ packet: inout Data, sequenceNumber: inout UInt16) -> PingReply {
        var reply = PingReply()
		guard packet.count >= ICMPHeader.size else {return reply}
	...
	number.
	- returns: true if the packet looks like a reasonable IPv4 ping response. */
	private func validatePingResponsePacket(_ packet: inout Data, sequenceNumber: inout UInt16) -> PingReply {
		switch hostAddressFamily {
	...
	fileprivate func readData() {
		...
		if bytesRead > 0 {
            ...
            let reply = validatePingResponsePacket(&packet, sequenceNumber: &sequenceNumber)
			
            delegate?.simplePing(self, didReceivePingResponsePacket: reply, sequenceNumber: sequenceNumber)
		} else {
